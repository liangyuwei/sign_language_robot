...
def main():
  ...
  try:
    ...
    moveit_commander.roscpp_initialize(sys.argv)
    rospy.init_node('yumi_sign_robot_track_dmp_trajs',
                    anonymous=True)
    ...
    ### Read h5 file for wrist trajectories
    f = h5py.File(file_name, "r")
    l_wrist_pos = f[group_name + "/l_wrist_pos_to_track"][:]
    r_wrist_pos = f[group_name + "/r_wrist_pos_to_track"][:]  
    l_wrist_quat = f[group_name + "/l_wrist_quat_resampled"][:]
    r_wrist_quat = f[group_name + "/r_wrist_quat_resampled"][:]      
    f.close()
    ...
    pose_target = left_arm_group.get_current_pose("yumi_link_7_l")
    pose_target.pose.position.x = l_wrist_pos[0, 0]
    pose_target.pose.position.y = l_wrist_pos[0, 1]
    pose_target.pose.position.z = l_wrist_pos[0, 2]
    # note that output from rotm2quat() is of the form (w,x,y,z), i.e. l_wrist_quat_resampled are so
    pose_target.pose.orientation.x = l_wrist_quat[0, 1]
    pose_target.pose.orientation.y = l_wrist_quat[0, 2] 
    pose_target.pose.orientation.z = l_wrist_quat[0, 3]
    pose_target.pose.orientation.w = l_wrist_quat[0, 0] # set the start pos
    left_arm_group.allow_replanning(True)
    left_arm_group.set_pose_target(pose_target, 'yumi_link_7_l') 
    left_arm_group.go(wait=True) # plan and go
    left_arm_group.stop() # stop
    left_arm_group.clear_pose_targets() # clear targets
    # Right arm
    print("Right Arm: Go to start pose before planning...")
    pose_target = right_arm_group.get_current_pose("yumi_link_7_r")
    pose_target.pose.position.x = r_wrist_pos[0, 0]
    pose_target.pose.position.y = r_wrist_pos[0, 1]
    pose_target.pose.position.z = r_wrist_pos[0, 2]
    # note that output from rotm2quat() is of the form (w,x,y,z), i.e. l_wrist_quat_resampled are so
    pose_target.pose.orientation.x = r_wrist_quat[0, 1]
    pose_target.pose.orientation.y = r_wrist_quat[0, 2] 
    pose_target.pose.orientation.z = r_wrist_quat[0, 3]
    pose_target.pose.orientation.w = r_wrist_quat[0, 0] # set the start pos
    right_arm_group.allow_replanning(True)
    right_arm_group.set_pose_target(pose_target, 'yumi_link_7_r') 
    right_arm_group.go(wait=True) # plan and go
    right_arm_group.stop() # stop
    right_arm_group.clear_pose_targets() # clear targets
    ...
    ### (IK) Compute Cartesian paths according to given pos and quat data
    # LEFT ARM
    ...
    for n in range(l_wrist_pos.shape[0]):
      wpose.position.x = l_wrist_pos[n, 0]
      wpose.position.y = l_wrist_pos[n, 1]
      wpose.position.z = l_wrist_pos[n, 2]
      wpose.orientation.x = l_wrist_quat[n, 1]
      wpose.orientation.y = l_wrist_quat[n, 2]
      wpose.orientation.z = l_wrist_quat[n, 3]
      wpose.orientation.w = l_wrist_quat[n, 0] # quat is (w,x,y,z), output from rotm2quat()
      waypoints.append(copy.deepcopy(wpose))
    tt0 = time.time()
    (new_plan_l, fraction) = left_arm_group.compute_cartesian_path(
                                        waypoints,   # waypoints to follow
                                        0.01, #0.01,        # eef_step # set to 0.001 for collecting the data
                                        0.0,     # jump_threshold
    ...
    waypoints = []
    wpose = geometry_msgs.msg.Pose()
    for n in range(r_wrist_pos.shape[0]):
      wpose.position.x = r_wrist_pos[n, 0]
      wpose.position.y = r_wrist_pos[n, 1]
      wpose.position.z = r_wrist_pos[n, 2]
      wpose.orientation.x = r_wrist_quat[n, 1]
      wpose.orientation.y = r_wrist_quat[n, 2]
      wpose.orientation.z = r_wrist_quat[n, 3]
      wpose.orientation.w = r_wrist_quat[n, 0] # quat is (w,x,y,z), output from rotm2quat()
      waypoints.append(copy.deepcopy(wpose))
    tt0 = time.time()
    (new_plan_r, fraction) = right_arm_group.compute_cartesian_path(
                                        waypoints,   # waypoints to follow
                                        0.01, #0.01,        # eef_step # set to 0.001 for collecting the data
                                        0.0,     # jump_threshold
                                        avoid_collisions=True)    
    ...
    print "============ Merge two plans of both arms..."
    new_plan_dual = moveit_msgs.msg.RobotTrajectory()
    new_plan_dual.joint_trajectory.header.frame_id = '/world'
    new_plan_dual.joint_trajectory.joint_names = ['yumi_joint_1_l', 'yumi_joint_2_l', 'yumi_joint_7_l', 'yumi_joint_3_l', 'yumi_joint_4_l', 'yumi_joint_5_l', 'yumi_joint_6_l'] \
    ...
    # order can be checked in yumi..moveit_config/config/fake_controllers.yaml
    ...
    dual_arms_group.execute(new_plan_dual, wait=True)
