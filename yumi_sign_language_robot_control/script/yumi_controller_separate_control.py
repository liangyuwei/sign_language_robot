...
class YumiControl():
  ...
  def __init__(self):
    ...
    self.left_arm_client = actionlib.SimpleActionClient("/yumi/left_arm_joint_controller/follow_joint_trajectory", FollowJointTrajectoryAction)
    self.left_arm_client.wait_for_server()
    self.right_arm_client = actionlib.SimpleActionClient("/yumi/right_arm_joint_controller/follow_joint_trajectory", FollowJointTrajectoryAction)
    ...
    # + ['Link1', 'Link11', 'Link2', 'Link22', 'Link3', 'Link33', 'Link4', 'Link44', 'Link5', 'Link51', 'Link52', 'Link53']
    self.left_arm_joint_names = ['yumi_joint_1_l', 'yumi_joint_2_l', 'yumi_joint_7_l', 'yumi_joint_3_l', 'yumi_joint_4_l', 'yumi_joint_5_l', 'yumi_joint_6_l'] 
  ...
  def arm_action_control(self, q_goal_l, q_goal_r, t_l, t_r):
    print("== Waiting for action server... ")
    self.left_arm_client.wait_for_server()
    self.right_arm_client.wait_for_server()
    # JointTrajectory
    left_joint_goal = self.set_arm_joint_goal(q_goal_l, t_l, True)
    right_joint_goal = self.set_arm_joint_goal(q_goal_r, t_r, False)    
    # FollowJointTrajectoryActionGoal
    left_action_goal = FollowJointTrajectoryActionGoal()
    left_action_goal.goal.trajectory = left_joint_goal
    right_action_goal = FollowJointTrajectoryActionGoal()
    right_action_goal.goal.trajectory = right_joint_goal
    # send goal
    self.left_arm_client.send_goal(left_action_goal.goal)
    self.right_arm_client.send_goal(right_action_goal.goal)
  ...
  def set_arm_joint_goal(self, q_goal, t, left_or_right):
    # init
    arm_cmd = JointTrajectory()
    waypoint = JointTrajectoryPoint()
    # joint names
    if left_or_right:
      arm_cmd.joint_names = self.left_arm_joint_names
    else:
      arm_cmd.joint_names = self.right_arm_joint_names
    # append waypoints
    waypoint.positions = q_goal
    waypoint.time_from_start.secs = t
...
def main():
  ...
  try:
    ...
    t_l = 2.0
    t_r = 3.0
    yumi_control.arm_action_control(q_arm_tpose_l, q_arm_hug_r, t_l, t_r)
