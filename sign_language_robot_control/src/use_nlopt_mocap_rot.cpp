...
typedef struct {
  ...
  Matrix3d upperarm_ori_goal;
  Matrix3d forearm_ori_goal;
  Matrix3d hand_ori_goal;
  ...
  unsigned int num_up_seg = 0; // upper arm link
  unsigned int num_fr_seg = 0; // forearm link
  unsigned int num_hd_seg = 0; // hand link
...
KDL::ChainFkSolverPos_recursive setup_kdl(my_constraint_struct &constraint_data)
{
  ...
  const std::string UPPERARM_LINK = "";
  const std::string FOREARM_LINK = "left_forearm_link";
  const std::string HAND_LINK = "left_ee_link";
  ...
  if (constraint_data.num_up_seg == 0 || constraint_data.num_fr_seg == 0 || constraint_data.num_hd_seg == 0) // if the IDs not set
  {
    ...
    for (unsigned int i = 0; i < num_segments; ++i){
      if (kdl_chain.getSegment(i).getName() == UPPERARM_LINK){
        constraint_data.num_up_seg = i;
        //ROS_INFO_STREAM("Elbow link found.");
        break;
      }
      if (kdl_chain.getSegment(i).getName() == FOREARM_LINK){
        constraint_data.num_fr_seg = i;
        //ROS_INFO_STREAM("Elbow link found.");
        break;
      }
      if (kdl_chain.getSegment(i).getName() == HAND_LINK){
        constraint_data.num_hd_seg = i;
        //ROS_INFO_STREAM("Elbow link found.");
        break;
      }
    }
...
int main(int argc, char **argv)
{
  ...
  const std::string in_file_name = "mocap_paths.h5";
  const std::string in_dataset_name = "left_up_fr_hd_rot_path_1";
  std::vector<std::vector<double>> read_up_fr_hd_path = read_h5(in_file_name, in_dataset_name); 
  // using read_h5() does not need to specify the size!!!
  // upperarm link rotation(9) + forearm link(9) + hand link(9) = 27
  unsigned int num_datapoints = read_up_fr_hd_path.size(); 
  ...
  for (int i = 0; i < 3; ++i)
  {
    for (int j = 0; j < read_up_fr_hd_path[i].size(); ++j)
      std::cout << read_up_fr_hd_path[i][j] << " ";
    std::cout << std::endl;
  }
  exit(0);
  */
  ...
  for (unsigned int it = 0; it < num_datapoints; ++it)
  {
    ...
    // Set goal point
    std::vector<double> path_point = read_up_fr_hd_path[it];
    std::vector<double> upperarm_ori(path_point.begin(), path_point.begin()+9); // 9-dim
    std::vector<double> forearm_ori(path_point.begin()+9, path_point.begin()+18); // 9-dim
    std::vector<double> hand_ori(path_point.begin()+18, path_point.begin()+27); // 9-dim
...
//    std::vector<double> elbow_pos(path_point.begin()+12, path_point.begin()+15); //end()); // 3-dim
    ...
    std::cout << "upperarm_ori.size() = " << upperarm_ori.size() << ", ";
    std::cout << "forearm_ori.size() = " << forearm_ori.size() << ", ";
    std::cout << "hand_ori.size() = " << hand_ori.size() << "." << std::endl; 
    exit(0);
    // convert
    Matrix3d upperarm_ori_goal = Map<Matrix<double, 3, 3, RowMajor>>(upperarm_ori.data(), 3, 3);
    Matrix3d forearm_ori_goal = Map<Matrix<double, 3, 3, RowMajor>>(upperarm_ori.data(), 3, 3);
    Matrix3d hand_ori_goal = Map<Matrix<double, 3, 3, RowMajor>>(upperarm_ori.data(), 3, 3);
    constraint_data.upperarm_ori_goal = upperarm_ori_goal;
    constraint_data.forearm_ori_goal = forearm_ori_goal;
    constraint_data.hand_ori_goal = hand_ori_goal;
    ...
    std::cout << std::endl << "Wrist pos is: " << constraint_data.upperarm_ori_goal << std::endl
                           << "Wrist rot is: " << constraint_data.forearm_ori_goal << std::endl
                           << "Elbow pos is: " << constraint_data.hand_ori_goal << std::endl;
    exit(0);
    //} 
  ...
  const std::string dataset_name = "mocap_ik_result_left_1";
