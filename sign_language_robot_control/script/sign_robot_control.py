...
def main():
  ...
  try:
    ...
    waypoints = []
    # Mid point: set position
    wpose = left_arm_group.get_current_pose().pose
    wpose.position.x = 0.426575
    wpose.position.y = 0.306569
    wpose.position.z = 0.389701
    # Mid point: set orientation
    tmp_pose = tf.transformations.quaternion_from_euler(1.4893, 0.0561, -1.5322)
    wpose.orientation.x = tmp_pose[0]
    wpose.orientation.y = tmp_pose[1]
    wpose.orientation.z = tmp_pose[2]
    wpose.orientation.w = tmp_pose[3]        
    waypoints.append(copy.deepcopy(wpose))
    # Final point: set position
    wpose.position.x = 0.307
    wpose.position.y = 0.2979
    wpose.position.z = 0.2708
    # Final point: set orientation
    tmp_pose = tf.transformations.quaternion_from_euler(1.5592, 0.02323, -1.4011)
    wpose.orientation.x = tmp_pose[0]
    wpose.orientation.y = tmp_pose[1]
    wpose.orientation.z = tmp_pose[2]
    wpose.orientation.w = tmp_pose[3]        
    waypoints.append(copy.deepcopy(wpose))
    # plan path
    (plan, fraction) = left_arm_group.compute_cartesian_path(
                                        waypoints,   # waypoints to follow
                                        0.01,        # eef_step
                                        0.0)         # jump_threshold
    # execute the plan
    print "============ Execute the planned path..."        
    left_arm_group.execute(plan, wait=True)
