...
void GetPosAndVisualize::posCallback(const arm_hand_capture::DualArmDualHandStateWithImageConstPtr& msg)
{
  ...
  marker.ns = "movement_after_transform"; // namespace?
  ...
  // Draw hands
  double robot_hand_length = 0.2; // for inspire-robot hand
...
GetPosAndVisualize::GetPosAndVisualize()
{
  ...
  pos_sub_ = n_.subscribe<arm_hand_capture::DualArmDualHandStateWithImage>("/dual_arms_dual_hands_state_with_image_after_transform", 100, boost::bind(&GetPosAndVisualize::posCallback, this, _1));
...
int main(int argc, char** argv){
  ...
  ros::init(argc, argv, "load_visualization_markers_after_transform_node");
