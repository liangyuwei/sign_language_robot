...
void poseCallback(const arm_hand_capture::DualArmDualHandStateWithImageConstPtr& msg){
  ...
  br.sendTransform(tf::StampedTransform(r_up_transform, timestamp, "world", "right_upperarm_original"));
  br.sendTransform(tf::StampedTransform(r_fr_transform, timestamp, "world", "right_forearm_original"));
  br.sendTransform(tf::StampedTransform(r_hd_transform, timestamp, "world", "right_hand_original"));

  br.sendTransform(tf::StampedTransform(l_up_transform, timestamp, "world", "left_upperarm_original"));
  br.sendTransform(tf::StampedTransform(l_fr_transform, timestamp, "world", "left_forearm_original"));
  br.sendTransform(tf::StampedTransform(l_hd_transform, timestamp, "world", "left_hand_original"));
...
int main(int argc, char** argv){
  ...
  ros::init(argc, argv, "unpack_original_movement_message_node");
  ros::NodeHandle n;
  ROS_INFO_STREAM("Waiting for /dual_arms_dual_hands_state_with_image_original_movement to come up...");
  ros::Subscriber sub = n.subscribe("/dual_arms_dual_hands_state_with_image_original_movement", 100, &poseCallback);
