...
void poseCallback(const arm_hand_capture::DualArmDualHandStateWithImageConstPtr& msg){
  ...
  br.sendTransform(tf::StampedTransform(r_up_transform, timestamp, "world", "right_upperarm_adjusted"));
  br.sendTransform(tf::StampedTransform(r_fr_transform, timestamp, "world", "right_forearm_adjusted"));
  br.sendTransform(tf::StampedTransform(r_hd_transform, timestamp, "world", "right_hand_adjusted"));

  br.sendTransform(tf::StampedTransform(l_up_transform, timestamp, "world", "left_upperarm_adjusted"));
  br.sendTransform(tf::StampedTransform(l_fr_transform, timestamp, "world", "left_forearm_adjusted"));
  br.sendTransform(tf::StampedTransform(l_hd_transform, timestamp, "world", "left_hand_adjusted"));
...
int main(int argc, char** argv){
  ...
  ros::init(argc, argv, "unpack_synced_message_with_image_node");
  ros::NodeHandle n;
  ROS_INFO_STREAM("Waiting for /dual_arms_dual_hands_state_with_image_adjusted_movement to come up...");
  ros::Subscriber sub = n.subscribe("/dual_arms_dual_hands_state_with_image_adjusted_movement", 100, &poseCallback);
